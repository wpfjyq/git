    ...
    <div id="container">
        <div class="user_bindSidebar">
            <dl id="user_sideBarmenu" class="user_sideBarmenu">
                <dt><h3>帐号设置</h3></dt>
                <dd><a href="?r=user/accountbind">帐号绑定</a></dd>
                <dd><a class="hover" href="?r=user/pwdup">修改密码</a></dd>
            </dl>
        </div>
        <input type="hidden" id="hasSidebar" value="1">	<div class="content user_modifyContent">
            <dl class="c_section">
                ...
                <dd>
                    <form  action="?r=user/pwdup" method="post" id="updatePswForm">
                        <table class="savePassword">
                            <tbody><tr>
                                <td>登录邮箱</td>
                                <td class="c7" title="<?php echo $user['user_id']?>"><?php echo $user['username']?></td>
                            </tr>
                            <tr>
...
<script>
    $(function(){
        ...
        $("#oldpassword").blur(function(){
                 var user_id=$(".c7").attr('title');
//            alert(user_id);
                var oldpassword=$(this).val();
                 if(oldpassword==""){
                    $("#old_error").html("<font color='red'>请填写原密码</font>");
                     validation.checkold=false;
                     return false;
                }else{
                    $.ajax({
                        type:"post",
...
//                            alert(str);
                            if(str==1){
                                $("#old_error").html("<font color='green'>原密码正确</font>");
                                validation.checkold=true;
                                return true;
                            }else{
                                $("#old_error").html("<font color='red'>与原密码不符</font>");
                                validation.checkold=false;
                                return false;
                            }
                        }
      ...
      //验证新密码
        $("#newpassword").blur(function(){
            ...
            var newpwd=$(this).val();
            if(newpwd==""){
                    $("#new_error").html("<font color='red'>密码不能为空</font>");
                validation.checknewpwd=false;
                return false;
            }else{
                if(!reg.test(newpwd)){
                    $("#new_error").html("<font color='red'>密码格式不正确，必须是6-16位(数字字母下划线)</font>");
                    validation.checknewpwd=false;
                    return false;
                }else{
                    $("#new_error").html("<font color='green'>密码合格</font>");
                    validation.checknewpwd=true;
                    return true;
                }
            }
        });
     ...
     //验证密码是否一致
        $("#comfirmpassword").blur(function(){
            ...
            if(newpsd==""){
                $("#news_error").html("<font color='red'>请输入确认密码</font>");
                validation.checknewpsd=false;
                return false;
            }else{
                if(newpsd!=newpwd){
                    $("#news_error").html("<font color='red'>确认密码不正确</font>");
                    validation.checknewpsd=false;
                    return false;
                }else{
                    $("#news_error").html("<font color='green'>合格</font>");
                    validation.checknewpsd=true;
                    return true;
                }

            }
        });
        $("#pwdup").submit(function(){
            $("#oldpassword").trigger("blur");
            $("#newpassword").trigger("blur");
            $("#comfirmpassword").trigger("blur");
            if(validation.checkold&&validation.checknewpsd&&validation.checknewpwd){
                return true;
            }else{
                return false;
            }
