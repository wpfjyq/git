...
<div id="body">
    ...
    <div id="container">
        <div class="sidebar">
            <a class="btn_create" href="?r=company/position">发布新职位</a>
            <dl class="company_center_aside">
                <dt>我收到的简历</dt>
                <dd>
                    <a href="?r=company/pending">待处理简历</a>
                </dd>
                <dd>
                    <a href="?r=company/d">待定简历</a>
                </dd>
                <dd>
                    <a href="?r=company/yi">已通知面试简历</a>
                </dd>
                <dd>
                    <a href="?r=company/no">不合适简历</a>
                </dd>
                <dd class="current btm">
                    <a href="?r=company/looks">已阅读</a>
                    <span>1</span>
                </dd>
            </dl>
            <dl class="company_center_aside">
        ...
        <ul class="reset resumeLists">
            <?php if(!empty($data)){?>
                <?php foreach($data as $k=>$v){?>
                    <li data-id="<?php echo $v['jianli_id']?>" class="onlineResume">
                        <label class="checkbox">
                            <input type="checkbox" title="<?php echo $v['jianli_id']?>" name="jian">
                            <i></i>
                        </label>


                        <div class="resumeShow">
                            <a title="预览在线简历" target="_blank" class="resumeImg" href="?r=company/preview&jianli_id=<?php echo $v['jianli_id']?>">
                                <?php if(!empty($v['img'])){?>
                            ...
                            <div class="resumeIntro">
                                <h3 class="unread">
                                    <a target="_blank" title="<?php echo $v['jian_name']?>历" href="?r=user/look?jianli_id=<?php echo $v['jianli_id']?>">
                                        <?php echo $v['jian_name']?>
                                    </a>
                                    <em></em>
                                </h3>
                                <span class="fr">投递时间：<?php echo date("Y-m-d H:s:i",$v['d_time'])?></span>
                                <div>
                                    <?php echo $v['name']?>  			                                        	 / <?php echo $v['sex']?> 			                                        	/ <?php echo $v['education']?> 			                                        	/ <?php echo $v['year']?>  	/ <?php echo $v['city']?> 			                                            			                                        		<br>
                                    <?php echo $v['position_name']?> · <?php echo $v['company_name']?> | <?php echo $v['nature']?> · <?php echo $v['s_name']?>
                                </div>
                                <div class="jdpublisher">
				                                        <span>
				                                        	应聘职位：<a title="随便写" target="_blank" href="http://www.lagou.com/jobs/149594.html"><?php echo $v['hope_work']?></a>
				                                       						                                        </span>
                                </div>
                            </div>
                            <div class="links">
                                <?php if($v['s']==2){?>
                                    <a data-deliverid="1686182" data-name="jason" data-positionid="149594" data-email="888888888@qq.com"  href="javascript:void(0)" title="<?php echo $v['jianli_id']?>" type="notice" name="resume" style="color: red;">已通知面试</a>
                                    <a data-deliverid="1686182"  href="javascript:void(0)" title="<?php echo $v['jianli_id']?>" type="no" name="resume">不合适</a>
                                    <a data-deliverid="1686182"  href="javascript:void(0)" title="<?php echo $v['jianli_id']?>" type="deng" name="resume">待定</a>
                                <?php }else if($v['s']==3){?>
                                    <a data-deliverid="1686182" data-name="jason" data-positionid="149594" data-email="888888888@qq.com" class="resume_notice" href="javascript:void(0)" title="<?php echo $v['jianli_id']?>" type="notice" name="resume">通知面试</a>
                                    <a data-deliverid="1686182" class="resume_refuse" href="javascript:void(0)" title="<?php echo $v['jianli_id']?>" type="no" name="resume" style="color: red">不合适</a>
                                    <a data-deliverid="1686182" class="resume_caninterview" href="javascript:void(0)" title="<?php echo $v['jianli_id']?>" type="deng" name="resume">待定</a>
                                <?php }else if($v['s']==4){?>
                                    <a data-deliverid="1686182" data-name="jason" data-positionid="149594" data-email="888888888@qq.com" class="resume_notice" href="javascript:void(0)" title="<?php echo $v['jianli_id']?>" type="notice" name="resume">通知面试</a>
                                    <a data-deliverid="1686182" class="resume_refuse" href="javascript:void(0)" title="<?php echo $v['jianli_id']?>" type="no" name="resume" >不合适</a>
                                    <a data-deliverid="1686182" class="resume_caninterview" href="javascript:void(0)" title="<?php echo $v['jianli_id']?>" type="deng" name="resume" style="color: red">待定,请耐心等待</a>
                ...
                <?php }?>
            <?php } else{?>
                <li class="onlineResume">没有已通知面试的简历</li>
    ...
    <script>
        $(function(){
            $("a[name='resume']").click(function(){
                var jianli_id=$(this).attr("title");
                var type=$(this).attr("type");
                obj=$(this);
//				alert(type);
                $.ajax({
                    type:"post",
                    url:"?r=company/notice",
                    data:{
                        "jianli_id":jianli_id,
                        "type":type
